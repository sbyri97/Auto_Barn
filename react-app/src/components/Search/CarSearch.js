...
export default function CarSearch({searchItem, results}) {
    const sessionUser = useSelector((state) => state.session.user)
    const cars = useSelector((state) => state.search?.searchedCars?.cars)
    ...
    return (
        <div>
            ...
            :
            (((searchItem !== "") && (!results)) ?
                <div className='no-cars'> No Results </div>
            :
                <div className='no-cars'>Please Search For A Vehicle</div>
